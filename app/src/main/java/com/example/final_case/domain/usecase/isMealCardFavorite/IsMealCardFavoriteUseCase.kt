package com.example.final_case.domain.usecase.isMealCardFavorite
...
class IsMealCardFavoriteUseCase @Inject constructor(
    private val repository: MealRepository
) {
    operator fun invoke(id: String) = flow {
        try {
            emit(Resource.Loading())
            emit(Resource.Success(repository.isMealCardAddedToFavorites(id)))
